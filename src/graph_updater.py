...
from PySide6.QtCore import Qt, Signal, QObject, QTimer, QMargins
...
class GraphUpdater(QObject):
    def __init__(self, plc_window, parent=None):
        ...
        # เชื่อมต่อสัญญาณจาก PLCWindow
        self.plc_window.data_updated.connect(self.handle_plc_data_update)

        # ตั้งค่า Timer
        self.timer.timeout.connect(self.request_plc_data)
        self.timer.start(1000)  # อัปเดตทุก 1 วินาที

    def request_plc_data(self):
        """ขอข้อมูลจาก PLC"""
        try:
            if hasattr(self.plc_window, 'update_data'):
                self.plc_window.update_data()
            else:
                logging.error("PLCWindow has no update_data method")
        except Exception as e:
            logging.exception("Error in request_plc_data")
