...
data_upload.py
==============
อัพโหลดและดึงข้อมูลการผลิตระหว่าง local app กับ MySQL database

หน้าที่หลัก:
  - ดึง product_name จาก lot_number (read DB)
  - ตรวจสอบสิทธิ์ผู้ใช้จาก tbl_training (write DB)
  - บันทึกผลการผลิต (shot count, defect, PM) ลง database
  - ใช้ DatabaseManager (Singleton) แทนการ connect ตรงเพื่อรองรับ retry

การใช้งาน:
  - สร้างโดย LoginManager และ MainWindow:
      self.data_uploader = DataUploader()
  - เรียก:
      name, err = data_uploader.get_product_name_by_lot(lot_number)
      ok, info  = data_uploader.check_user_permission(user_id)
      data_uploader.upload_result(...)

ความสัมพันธ์กับโมดูลอื่น:
  - database_manager : จัดการ connection read/write
  - config_manager   : อ่าน path สำหรับ log file
  - LoginManager     : เรียกเพื่อ validate lot + permission ก่อน login
"""
